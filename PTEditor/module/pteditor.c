...
static int update_vm(ptedit_entry_t* new_entry, int lock) {
  ...
  flush_icache_page(vma, pte_page(*old_entry.pte));
...
#if defined(__riscv)
        if(!mm || (mm && !mm->pgd)) {
            asm volatile("csrr %0, satp" : "=r" (paging.root));
            paging.root &= 0xfffffffffff;
            (void)to_user((void*)ioctl_param, &paging, sizeof(paging));
            return 0;
        }
#endif
...
MODULE_PARM_DESC(addr, "Address of the kallsyms_lookup_name symbol");
