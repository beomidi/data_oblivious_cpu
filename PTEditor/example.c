...
int main(int argc, char *argv[]) {
  size_t address_pfn, target_pfn, address_pfn_updated;
  (void)argc;
  (void)argv;
  if(ptedit_init()) {
...
//   ptedit_use_implementation(PTEDIT_IMPL_KERNEL);
  ...
  void *address = mmap(0, 4096, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS | MAP_NORESERVE, -1, 0);
  memset(address, 'A', 4096);

  void *target = mmap(0, 4096, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS | MAP_NORESERVE, -1, 0);
