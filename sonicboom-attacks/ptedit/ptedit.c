...
#elif defined(__riscv)
    if (line == 0 || line == 3) printf("------------------+-+-+-+-+-+-+-+-+--+--+-+-+-+\n");
    if (line == 1) printf("       PFN        |RSW|D|A|G|U|X|W|R|V|\n");
    if (line == 2) {
        printf("|");
        printf(" %16p |", (void*)((entry >> 10) & ((1ull << 44) - 1)));
        PEDIT_PRINT_B("%d", PTEDIT_B(entry, PTEDIT_PAGE_BIT_RSW2 << 1) | PTEDIT_B(entry, PTEDIT_PAGE_BIT_RSW1));
        PEDIT_PRINT_B("%d", PTEDIT_B(entry, PTEDIT_PAGE_BIT_D));
        PEDIT_PRINT_B("%d", PTEDIT_B(entry, PTEDIT_PAGE_BIT_A));
        PEDIT_PRINT_B("%d", PTEDIT_B(entry, PTEDIT_PAGE_BIT_G));
        PEDIT_PRINT_B(" %d", PTEDIT_B(entry, PTEDIT_PAGE_BIT_U));
        PEDIT_PRINT_B(" %d", PTEDIT_B(entry, PTEDIT_PAGE_BIT_X));
        PEDIT_PRINT_B("%d", PTEDIT_B(entry, PTEDIT_PAGE_BIT_W));
        PEDIT_PRINT_B("%d", PTEDIT_B(entry, PTEDIT_PAGE_BIT_R));
        PEDIT_PRINT_B("%d", PTEDIT_B(entry, PTEDIT_PAGE_BIT_V));
        printf("\n");
    }
#endif
}
...
ptedit_fnc int ptedit_init() {
    ...
    if(ptedit_get_pagesize() == 16384) {
        ...
        ptedit_paging_definition.pmd_entries = 9;
        ptedit_paging_definition.pt_entries = 9;
        ptedit_paging_definition.page_offset = 10;
...
// ---------------------------------------------------------------------------
size_t ptedit_apply_nouarch(size_t entry) {
 	entry &=~(1ull << 63);
        entry |= (1ull << 63);
        return entry;
}
